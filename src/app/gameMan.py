...
from app import backup, optionWindow, tk_tools
import loadScreen
import packages
import editoritems
import utils
		...
		if os.path.exists( tag_loc ) and not found_tag:
			...
			try:
				...
				wx.MessageDialog(
					...
					caption=_( 'BEE2 - Aperture Tag Files Missing' ),
...
def add_game( e=None, refresh_menu=True ):
	...
	wx.MessageDialog(
		parent=wx.GetApp(),
		caption=_( 'BEE2 - Add Game' ),
		message=_( 'Select the folder where the game executable is located ({appname})...' )
	...
	diag = wx.FileDialog(
		parent=None,
		message=_( 'Find Game Exe' ),
		defaultDir='C:/Program Files (x86)/Steam/steamapps/common/Portal 2'
		if Path('C:/Program Files (x86)/Steam/steamapps/common/Portal 2').exists() else 'C:/',
		wildcard=f'{_( "Executable" )} (*.exe)|*.exe ',
	...
	while True:
		diag = wx.TextEntryDialog(
			parent=wx.GetApp(),
			message=_( "Enter the name of this game:" ),
			caption=_( 'BEE2 - Add Game' ),
			value=name,
			style=wx.CENTER
		)
...
def add_menu_opts( menu: wx.Menu, callback=None ):
	...
	for ind in range( menu.GetMenuItemCount(), 0, -1 ):
		...
		if menu.FindItemByPosition( ind ).IsRadio():
			menu.DestroyItem( menu.FindItemByPosition( ind ) )

	for val, game in enumerate( all_games ):
		item = menu.AppendRadioItem(
			id=None,  # TODO: add a function to get an available menu index
			item=game.name,
			help=f'Change the target game to {game.name}'
		)
		menu.Bind(wx.EVT_MENU, updateGame, item)
	updateGame()


def updateGame(evt: wx.MenuEvent):
	pass
