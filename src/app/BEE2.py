...
from app import gameMan, paletteLoader, UI, music_conf, logWindow, img, tk_error
import loadScreen
import packages
import utils
...
class App(wx.App):
	...
	def __init__(self):
		...
		wx.GetApp().TK_ROOT.report_callback_exception = tk_error
		...
		if utils.MAC:
			wx.GetApp().TK_ROOT.lift()

		logging_win = logWindow.LogWindow(
			GEN_OPTS.get_bool( 'Debug', 'show_log_win' ),
			GEN_OPTS.get( 'Debug', 'window_log_level', fallback='INFO' )
		)
		...
		pack_data, package_sys = packages.load_packages(
			GEN_OPTS[ 'Directories' ][ 'package' ] if not utils.DEV_MODE else './run/packages',
		...
		wx.GetApp().TK_ROOT.tk.call( 'after', 10, 'raise', wx.GetApp().TK_ROOT )
